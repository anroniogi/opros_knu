...
int32_t SerialCommunication::Initialize(OPRoS::Property parameter)
{
...
int32_t SerialCommunication::Finalize(void)
{
	if(handle != NULL) {
		handle->CloseSerial();
		delete handle;
...
int32_t SerialCommunication::Enable(void)
{
	if(handle == NULL) {
		return API_ERROR;
	}
...
int32_t SerialCommunication::Disable(void)
{
	if(handle == NULL) {
		return API_ERROR;
	}
...
int32_t SerialCommunication::SetProperty(OPRoS::Property parameter)
{
	if(parameter.FindName("TimeOut") == false)		return API_ERROR;
	unsigned long timeOut = (unsigned long)atol(parameter.GetValue("TimeOut").c_str());
	if(parameter.FindName("BaudRate") == false)	return API_ERROR;
...
int32_t SerialCommunication::GetProperty(OPRoS::Property &parameter)
{
